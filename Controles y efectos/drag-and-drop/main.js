...
class DragList{
    constructor(list_selector, items_selector = "li") {
        ...
        this.items = document.querySelectorAll(items_selector);
    ...
    compareElement(item, ev) {
        ...
        this.finalElementHover = item;
            ...
            if (result == 1)
                this.list.insertBefore(this.fakeElement,item.nexSibling)
    ...
    handleDragEnd(ev) {
        ...
        if (this.finalPosition == 1)
            this.list.insertBefore(el, this.finalElementHover.nexSibling);
